...
/**
 * GridFiller acts roughly like fill tool in paint application
 ...
 */
public abstract class GridFiller<T> extends Grid<T>
{
    ...
    public GridFiller(int width, int heigth, Strategy strategy)
    {
        this(width, heigth, true, strategy);
    }
    public GridFiller(int width, int heigth, boolean boxed, Strategy strategy)
    {
        super(width, heigth, boxed);
        this.stackSize = 2*(width+heigth);
        this.strategy = strategy;
    }
    public static final <T> void allDirections(int x, int y, T color, GridFiller<T> filler)
    {
    ...
    public static final <T> void roundedSquare(int x, int y, T color, GridFiller<T> filler)
    {
    ...
    public static final <T> void square(int x, int y, T color, GridFiller<T> filler)
    {
        boolean se = filler.hit(x-1, y+1, color);   // south east
        boolean ne = filler.hit(x-1, y-1, color);   // north east
        boolean nw = filler.hit(x+1, y-1, color);   // north west
    ...
    private void push(int x, int y)
    {
        ...
        if (position < 0 || position >= length || visited.get(position))
        {
            return;
        }
    ...
    {
        void apply(int x, int y, T color, GridFiller<T> filler);
    }
}
