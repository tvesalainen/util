 ...
 */
public class SimplePoint implements Point, Serializable
{
    ...
    {
        return new SimplePoint(k*p.getX(), k*p.getY());
    }
    /**
    ...
    {
        SimplePoint res = new SimplePoint();
        for (Point pp : points)
        {
            res.x += pp.getX();
            res.y += pp.getY();
    ...
    {
        return new SimplePoint(p1.getX() - p2.getX(), p1.getY() - p2.getY());
    }
    /**
    ...
    public static final Point[] midPoints(int count, Point p1, Point p2)
    {
        ...
        Point gap = SimplePoint.subtract(p2, p1);
        Point leg = SimplePoint.mul((double)(1)/(double)(count+1), gap);
        for (int ii=0;ii<count;ii++)
        {
            res[ii] = SimplePoint.add(p1, SimplePoint.mul(ii+1, leg));
        }
        return res;
    }
    /**
